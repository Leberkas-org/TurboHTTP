...
/// </remarks>
public sealed class EncoderStageBufferSpec : StreamTestBase
{
    private async Task<(byte[] bytes, int written)> Encode11Async(HttpRequestMessage request)
    {
        ...
        foreach (var item in items)
        {
            ...
            allBytes.AddRange(data.Span.ToArray());
            written += data.Length;
            data.Dispose();
        }
    ...
    [Fact(Timeout = 10_000)]
    public async Task EncoderStageBuffer_should_write_small_byte_count_when_request_is_small()
    {
    ...
    [Fact(Timeout = 10_000)]
    public async Task EncoderStageBuffer_should_encode_without_overflow_when_request_body_is_large()
    {
    ...
    [Fact(Timeout = 10_000)]
    public async Task EncoderStageBuffer_should_encode_all_requests_correctly_when_requests_are_sequential()
    {
        ...
        for (var i = 0; i < requestCount; i++)
        {
            ...
            var data = (NetworkBuffer)items[0];
            try
            {
                ...
                var encoded = System.Text.Encoding.ASCII.GetString(data.Span);
    ...
    [Fact(Timeout = 5000)]
    public async Task EncoderStageBuffer_should_pass_through_body_unchanged_when_content_is_binary()
    {
    ...
    [Fact(Timeout = 10_000)]
    public async Task EncoderStageBuffer_should_encode_correctly_when_Http11_encoder_has_custom_input_buffer()
    {
    ...
    [Fact(Timeout = 10_000)]
    public async Task EncoderStageBuffer_should_encode_correctly_when_Http10_encoder_has_custom_input_buffer()
    {
