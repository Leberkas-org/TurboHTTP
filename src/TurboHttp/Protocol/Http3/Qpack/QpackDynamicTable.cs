...
public sealed class QpackDynamicTable
{
    ...
    public QpackEntry? GetEntry(int absoluteIndex)
    {
        ...
        // Walk from the front (oldest) since absolute indices increase
        var node = _entries.First;
        while (node is not null)
        {
            if (node.Value.AbsoluteIndex == absoluteIndex)
            {
                return node.Value.Entry;
            }

            // Entries are ordered by absolute index; if we've passed the target, it was evicted
            if (node.Value.AbsoluteIndex > absoluteIndex)
            {
                return null;
            }
