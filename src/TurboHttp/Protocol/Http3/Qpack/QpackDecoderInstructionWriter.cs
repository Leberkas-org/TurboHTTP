...
public static class QpackDecoderInstructionWriter
{
    ...
    public static void WriteSectionAcknowledgment(int streamId, IBufferWriter<byte> output)
    {
        if (streamId < 0)
        {
        ...
        QpackIntegerCodec.Encode(streamId, 7, 0x80, output);
    }
    ...
    /// <param name="output">Destination buffer writer.</param>
    public static void WriteStreamCancellation(int streamId, IBufferWriter<byte> output)
    {
        if (streamId < 0)
        {
            throw new ArgumentOutOfRangeException(nameof(streamId), "Stream ID must be non-negative.");
        ...
        QpackIntegerCodec.Encode(streamId, 6, 0x40, output);
    }
    ...
    /// <param name="output">Destination buffer writer.</param>
    public static void WriteInsertCountIncrement(int increment, IBufferWriter<byte> output)
    {
        if (increment <= 0)
        {
        ...
        QpackIntegerCodec.Encode(increment, 6, 0x00, output);
    }
}
