...
public sealed class HpackEncoder
{
    ...
    /// For the common non-Huffman path, encodes directly into the IBufferWriter span
    /// to avoid a temporary byte[] allocation.
    /// </summary>
    private static void WriteString(string value, IBufferWriter<byte> output, bool useHuffman)
    {
        ...
        if (useHuffman)
        {
            // Huffman path: need a temporary raw-bytes buffer for HuffmanCodec.
            // Use ArrayPool to avoid allocation.
            var rawBuf = ArrayPool<byte>.Shared.Rent(rawLength);
            try
            {
                Encoding.UTF8.GetBytes(value, rawBuf);
                var huffmanBytes = HuffmanCodec.Encode(rawBuf.AsSpan(0, rawLength));

                // RFC 7541 §5.2: only use Huffman if it actually saves bytes
                if (huffmanBytes.Length < rawLength)
                {
                    // Huffman flag: H bit = 1
                    WriteInteger(huffmanBytes.Length, prefixBits: 7, prefixFlags: 0x80, output);
                    var huffSpan = output.GetSpan(huffmanBytes.Length);
                    huffmanBytes.CopyTo(huffSpan);
        ...
        // Non-Huffman (or Huffman rejected): encode directly into the output span — zero allocation.
        WriteInteger(rawLength, prefixBits: 7, prefixFlags: 0x00, output);
        var rawSpan = output.GetSpan(rawLength);
        Encoding.UTF8.GetBytes(value, rawSpan);
        output.Advance(rawLength);
    }
