...
public sealed class ConnectionIntegrationTests
{
    ...
    [Fact(DisplayName = "Conn-H2-001: Sequential requests reuse same HTTP/2 connection")]
    public async Task Sequential_Requests_Reuse_Same_Connection()
    {
    ...
    [Fact(DisplayName = "Conn-H2-002: Concurrent requests multiplexed over single HTTP/2 connection")]
    public async Task Concurrent_Requests_Multiplexed_Over_Single_Connection()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
    ...
    [Fact(DisplayName = "Conn-H2-003: Binary body POST echoed correctly over HTTP/2")]
    public async Task Binary_Body_Post_Echoed_Over_Http2()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
    ...
    [Fact(DisplayName = "Conn-H2-004: Multiple different endpoints served on same HTTP/2 connection")]
    public async Task Multiple_Endpoints_On_Same_Connection()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
    ...
    [Fact(DisplayName = "Conn-H2-005: POST with body followed by GET on same HTTP/2 connection")]
    public async Task Post_Followed_By_Get_On_Same_Connection()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
