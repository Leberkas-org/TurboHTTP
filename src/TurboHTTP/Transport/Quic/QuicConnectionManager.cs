...
internal sealed class QuicConnectionManager : IAsyncDisposable
{
    ...
    /// Stream-level concurrency is handled by the QUIC transport's built-in flow control;
    /// no C# semaphore is used so multiple requests open their streams independently.
    /// </summary>
    ...
    /// <summary>
    /// Creates the shared <see cref="QuicClientProvider"/> on first use (thread-safe).
    /// </summary>
