...
public sealed class Http2RequestEncoder(bool useHuffman = false, int maxFrameSize = 16384)
{
    ...
    private readonly bool _useHuffman = useHuffman;
    ...
    public (int StreamId, IReadOnlyList<Http2Frame> Frames) Encode(HttpRequestMessage request, int streamId)
    {
        ...
        var hpackBytesWritten = _hpack.Encode(_reusableHeaders, ref hpackWritable, _useHuffman);
        ...
        var totalRead = 0;
        var dataFrameStartIndex = frames.Count;
        ...
        while (totalRead < bytesToSend)
        {
            ...
            frames.Add(new DataFrame(streamId, owner, bytesRead, endStream: false));
            totalRead += bytesRead;
        }
        ...
        if (frames.Count > dataFrameStartIndex)
        {
            var lastIdx = frames.Count - 1;
            var last = (DataFrame)frames[lastIdx];
            frames[lastIdx] = new DataFrame(streamId, last.Data, endStream: true);
        }
        else
        {
